...
{
    public static HashSet<Vector2Int> SimpleRandomWalk(Vector2Int startnPosition, int walkLength)
    {
        HashSet<Vector2Int> path = new HashSet<Vector2Int>();

        path.Add(startnPosition);
        var prevoriousPosition = startnPosition;

        for (int i = 0; i < walkLength; i++)
        {
            var newPosition = prevoriousPosition + Direction2D.GetRandomCardinalDirection();
            path.Add(newPosition);
            prevoriousPosition = newPosition;
        }
        return path;
